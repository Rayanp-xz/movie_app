...
class TapBarMovie extends StatefulWidget {
  const TapBarMovie({super.key,required this.genreId});
 final int genreId;
  @override
  State<TapBarMovie> createState() => _TapBarMovieState();
}

class _TapBarMovieState extends State<TapBarMovie> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body:FutureBuilder<MovieModel>(future: HttpRequest.getDiscoverMovies(widget.genreId),
  ...
  Widget MovieGenreWidret(MovieModel data){
      List<Movie>? movies=data.movies;
       if(movies!.isEmpty){
        return SizedBox(
          height: 200,
          child: Center(child: Text( 'No Movies found',style: TextStyle(color: Style.textColor,fontSize: 20),)),
        );
       }else{
        return Container(
          height: 270,
